...
import apis.google_sheet as gs
import apis.chadsoft as cd
...
def update_3lap_leaderboards(wks: gspread.worksheet.Worksheet, debug: bool = True) -> None:
    ...
    for ID, LM in zip(IDs, LMs):
        row += 1
        if ID == "noID" or ID == "":
            if debug: print(f"[PLAYER SKIPPED AT ROW {row}]")
            continue
        ID, jolly = gs.get_jolly_and_purify_ID(ID)
        
        if debug: print(f"[CHECKING ROW {row}] ID: {ID}")

        cd_LM = cd.get_player_last_modified(ID)
        if LM != "" and datetime.datetime.fromisoformat(LM) > cd_LM:
            continue

        if debug: print("  [OUTDATED DATA FOUND] fetching player JSON from Chadsoft...")
        ...
        for g in ghosts:
            ...
            if gs_track_column == "INVALID_TRACK_CATEGORY":
                if debug: print(f"  [SKIPPING INVALID TRACK CATEGORY] {g['trackName']}; category: {cd.get_category(categoryId)}")
                continue
            ...
            if debug: print("  (NEW GHOSTS FOUND)", g["trackName"] + "; category:", cd.get_category(categoryId) + "; time:", new_time)
        ...
        if from_last_gs_update >= gs.NUM_ROWS_PARTIAL_UPDATE:
            ...
            full_gs = gs.get_all_values(wks)
            if debug: print(f"[SUCCESSFUL] UPDATED {gs.NUM_ROWS_PARTIAL_UPDATE} ROWS OF GOOGLE SHEETS, PROCEEDING WITH THE NEXT BLOCK...")

    gs.set_all_values(wks, full_gs)

def update_unrestricteds_and_checks(wks: gspread.worksheet.Worksheet, debug: bool = True) -> None:
    full_gs = gs.get_all_values(wks)
...
if __name__ == "__main__":
    try:
        ...
        update_3lap_leaderboards(wks)
        update_unrestricteds_and_checks(wks)
